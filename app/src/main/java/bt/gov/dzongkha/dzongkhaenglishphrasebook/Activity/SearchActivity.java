...
public class SearchActivity extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //setting second toolbar containing back and playall button
...
//        });
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (id){
            case R.id.ac:
                ...
                //calling viewFavorites function
                viewFavorite();
                return true;
    ...
    public void displaySearchedPhrases(){
        ...
        //checking whether the string is english
        if(isEnglishWord(search_value)){
            Cursor cursor = db.rawQuery("select * from categoryDetails where en_phrase like '%"+search_value+"%'",null);
            ...
            {
                Toast.makeText(getApplicationContext(),"No related phrases found",Toast.LENGTH_SHORT).show();
            }
            cursor.close();
            db.close();
        ...
        //if the text is dzongkha
        else {
            Cursor cursor = db.rawQuery("select * from categoryDetails where dzo_phrase like '%"+search_value+"%'",null);
           // Cursor cursor = db.rawQuery("select * from categoryDetails where dzo_phrase  '+search_value+'",null);
            if (cursor.getCount() != 0)
            {
                if (cursor.moveToFirst())
                {
            ...
            for (int i = 0; i < dzo_phrases_list.size(); i++) {
                getDzongkhaPhrasesArrayList.add(
            ...
            getDzoPhrasesAdapter = new GetDzoPhrasesAdapter(getApplicationContext(),getDzongkhaPhrasesArrayList,getCategoryDetailIDList,ts.getTextSize(),ts.getDzo_size());
            recyclerView.setAdapter(getDzoPhrasesAdapter);
        }
    }

    //function to check whether the text entered is english or not
            ...
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                ...
                if (progressValue == 0) {
                    ...
                    if(isEnglishWord(search_value)) {
                        ...
                        getDzoPhrasesAdapter.changeTextSize(21,55);
                }
            }
